...
GLuint gfx::BufferManager::CreateBuffer(const std::string& name, GLenum type, int size, GLenum usage)
{
...
GLuint gfx::BufferManager::CreateBufferWithData(const std::string& name, GLenum type, int size, GLenum usage, void* data)
{
	auto it = m_Buffers.find(name);
	if(it != m_Buffers.end())
	{
...
void gfx::BufferManager::BindBufferToProgram(const std::string& name, ShaderProgram* program, GLuint bindingpoint)
{
	auto it = m_Buffers.find(name);
	if(it == m_Buffers.end())
	{
	...
	{
		if(m_ProgramBindings[program->GetHandle()].at(i) == bindingpoint)
			return;
	}
...
void gfx::BufferManager::BindBufferWithRange(const std::string& name, ShaderProgram* program, GLuint bindingpoint, int size)
{
	auto it = m_Buffers.find(name);
	if(it == m_Buffers.end())
	{
...
void gfx::BufferManager::ResizeBuffer(const std::string& name, int newsize)
{
	auto it = m_Buffers.find(name);
	if(it == m_Buffers.end())
	{
...
void gfx::BufferManager::UpdateBuffer(const std::string& name, int offset, void* data, int size)
{
	if(data == nullptr)
		return;
...
void* gfx::BufferManager::MapBuffer(const std::string& name, GLenum usage)
{
